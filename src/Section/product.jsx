    ...
    <section className="bg-[#FEFFF4] px-4 sm:px-12 xl:px-32 py-20 flex flex-col gap-16">
      ...
      <div className="mt-4 flex flex-wrap justify-center gap-4">
        {filters.map((label, i) => (
          <button
            key={label}
            className={`capitalize px-8 py-3 rounded-full text-sm font-medium ${
              i === 0
                ? "bg-[#2D3B36] text-white"
                : "border border-[#E3E3E3] text-black"
            }`}
          >
            {label}
          </button>
      ...
      <div
        ref={scrollRef}
        className="flex gap-8 overflow-x-auto scroll-smooth pb-4 px-2 hide-scrollbar"
      >
        {products.map((p, i) => (
          <div
            key={i}
      ...
      <div className="flex justify-center gap-4 lg:hidden">
        <button onClick={() => scroll("left")}>
          <img
            src={arrowLeft}
            alt="left"
            className="w-10 h-10 border rounded-full p-2 bg-white"
          />
        </button>
        <button onClick={() => scroll("right")}>
          <img
